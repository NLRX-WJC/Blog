...
// 添加路由
router
  .get("/users", async (ctx: Koa.Context, next: () => Promise<any>) => {
    // 获取用户
  })
  .post("/users", async (ctx: Koa.Context, next: () => Promise<any>) => {
    // 增加新用户
  })
  .put("/users/:id", async (ctx: Koa.Context, next: () => Promise<any>) => {
    // 修改参数 id 对应的用户
  })
  .del("/users/:id", async (ctx: Koa.Context, next: () => Promise<any>) => {
    // 删除参数 id 对应的用户
  });
